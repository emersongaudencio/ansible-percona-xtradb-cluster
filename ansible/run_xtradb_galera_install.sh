...
if [ "${VAR_MYSQL_VERSION}" == '' ] ; then
  echo "No Percona XtraDB version specified. Please have a look at README file for futher information!"
  exit 1
fi
...
ansible-playbook -v -i $SCRIPT_PATH/hosts -e "{mysql_version: '$VAR_MYSQL_VERSION', gtid: '$VAR_GTID', primary_server: '$VAR_PRIMARY_SERVER', galera_cluster_name: '$VAR_GALERA_CLUSTER_NAME', galera_cluster_address: '$VAR_GALERA_CLUSTER_ADDRESS'}" $SCRIPT_PATH/playbook/xtradb_install_galera.yml -l $VAR_HOST
